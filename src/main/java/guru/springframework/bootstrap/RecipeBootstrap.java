...
/**
 * Created by mimitic on 02/07/2018.
 */
...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<Recipe> getRecipes() {
        ...
        List<Recipe> recipesList = new ArrayList<>(2);  // two recipes
        ...
        // get categories
        Optional<Category> amerCatOptional = categoryRepository.findByDescription("American");
        if (!amerCatOptional.isPresent()) throw new RuntimeException("Expected Category not found!");

        Optional<Category> mexCatOptional = categoryRepository.findByDescription("Mexican");
        if (!mexCatOptional.isPresent()) throw new RuntimeException("Expected Category not found!");

        Category americanCategory = amerCatOptional.get();
        Category mexicanCategory = mexCatOptional.get();
        ...
        // set categories for taco
        tacosRecipe.getCategories().add(americanCategory);
        tacosRecipe.getCategories().add(mexicanCategory);

        // add to return list
        recipesList.add(tacosRecipe);
